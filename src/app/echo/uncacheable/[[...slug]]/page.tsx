...
export default async function EchoUncacheablePage({ params }: PageProps) {
  const { slug } = await params;
  const requestedPath = `/echo/uncacheable/${slug ? slug.join("/") : ""}`;
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-zinc-950 text-zinc-50 p-6 font-sans selection:bg-rose-500/30 relative">
      {/* Background gradients for rich aesthetics */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none">
        <div className="absolute -top-[40%] -left-[20%] w-[80%] h-[80%] rounded-full bg-rose-900/10 blur-[120px]" />
        <div className="absolute -bottom-[40%] -right-[20%] w-[80%] h-[80%] rounded-full bg-purple-900/10 blur-[120px]" />
      </div>
      ...
      <div className="relative z-10 w-full max-w-2xl">
        ...
        {/* Header */}
        <div className="mb-8">
          <span className="px-3 py-1 text-xs font-semibold font-mono tracking-wider text-rose-400 bg-rose-950/50 border border-rose-800/30 rounded-full inline-flex items-center gap-1.5 mb-3">
          ...
          <h1 className="text-4xl font-bold tracking-tight bg-gradient-to-r from-zinc-100 via-zinc-200 to-zinc-400 bg-clip-text text-transparent">
            Echo Uncacheable Endpoint
          </h1>
          <p className="text-zinc-400 mt-2">
            This page is configured with strict Cache-Control headers to prevent caching. Every request will hit the server and return a fresh response.
          </p>
        </div>
        ...
        <div className="rounded-2xl border border-zinc-800/50 bg-zinc-900/40 p-6 backdrop-blur-xl shadow-2xl space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center justify-between rounded-xl bg-zinc-950/80 px-4 py-3 border border-zinc-800/60">
              <code className="font-mono text-rose-400 break-all">{requestedPath}</code>
          ...
          <div className="space-y-1.5">
            ...
            <div className="rounded-xl bg-zinc-950/80 px-4 py-3 border border-zinc-800/60 font-mono text-purple-400 break-all text-lg font-bold">
              {uuid}
            </div>
          </div>
          ...
          <div className="border-t border-zinc-800/80 pt-6 space-y-3">
            <h3 className="text-sm font-semibold text-zinc-300">How to verify it is uncacheable:</h3>
            <ul className="text-xs text-zinc-400 space-y-2 list-disc pl-4 leading-relaxed">
              <li>
                Observe the <span className="text-purple-400 font-semibold">UUID</span> and <span className="text-rose-400 font-semibold">Timestamps</span> above.
              </li>
              <li>
                Refresh the page. Because caching is disabled, the values will <span className="text-zinc-200 font-semibold">change on every single refresh</span>.
              </li>
              <li>
                Inspect your browser developer tools Network tab to verify the response headers match the values below.
              </li>
            </ul>
          </div>
        </div>
        ...
        <div className="mt-6 rounded-xl border border-zinc-800/30 bg-zinc-950/40 p-4 font-mono text-xs text-zinc-500 space-y-1">
          ...
          <div><span className="text-zinc-400">Cache-Control:</span> private, no-cache, no-store, max-age=0, must-revalidate</div>
